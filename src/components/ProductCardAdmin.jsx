...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  console.log("Image array from data:", data?.image);
  return (
    <div className='w-36 p-4 bg-white rounded'>
        <div>
            {/* <img
                // src={`http://localhost:5000/${data?.image?.[0]}`}
                src={`http://localhost:5000/${data?.image?.[0]}`}
              ...
              <img
                  src={data?.image?.[0]?.startsWith("http") ? data.image[0] : `http://localhost:5000/${data.image[0]}`}
                  alt={data?.name || 'Product'}
                  className="w-full h-32 object-contain bg-gray-100"
                />
        </div>
